...
void loop() {
  ...
  int length = 0;
  length = sizeof(nuidPICC);
  
  char tagArr0[length + 1];
  char tagArr1[length + 1];
  char tagArr2[length + 1];
  char tagArr3[length + 1];
  char tagArr[12];

  dtostrf(nuidPICC[0],2,0, tagArr0);
  dtostrf(nuidPICC[1],2,0, tagArr1);
  dtostrf(nuidPICC[2],2,0, tagArr2);
  dtostrf(nuidPICC[3],2,0, tagArr3);
  ...
  client.publish("tagnumber", tagArr);
